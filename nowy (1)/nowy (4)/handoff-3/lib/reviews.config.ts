// handoff-3/lib/reviews.config.ts
...
export interface Review {
  id: string;
  author: string;          // np. "Marta K." (inicjały dla RODO)
  location?: string;       // np. "Kraków"
  petName?: string;        // np. "Burek"
  petType: 'dog' | 'cat' | 'other';
  problem: string;         // krótki tag problemu, np. "reaktywność na smyczy"
  rating: 1 | 2 | 3 | 4 | 5;
  date: string;            // ISO format YYYY-MM-DD
  text: string;            // treść opinii
  source: 'google' | 'direct' | 'email';
  consultationType?: 'kwadrans' | 'standardowa' | 'wyjazdowa';
  highlight?: boolean;     // true = pokaż na samej górze listy
}
...
export const reviews: Review[] = [
  ...
  },
  {
    ...
    rating: 5,
    date: '2025-01-10',
    text: 'Konsultacja wyjazdowa — Krzysztof przyjechał, zobaczył środowisko, zaobserwował Borisa. Plan na pół roku rozłożony na etapy. Konkret, zero ogólników.',
    source: 'direct',
    consultationType: 'wyjazdowa',
  },
];

// Aggregowana ocena — automatycznie wyliczona, ale możesz nadpisać
